...
function DoneCards() {
  ...
  const doneTasks = tasks.filter((t) => t.status === "done" && !t.archive);
  ...
  return (
    <>
      <div className="w-63 h-147 flex flex-col gap-4">
        <div className="w-full h-10 rounded-2xl border-t-2 border-[#40C057] shadow-md/10 p-2 flex items-center justify-between">
        ...
        <div className="w-full h-auto flex flex-col gap-3">
          {doneTasks.map((dt) => {
            const priorityIcon = showPriorityIcon(dt.priority);
            return (
